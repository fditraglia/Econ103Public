...
\begin{solution}[1.5in]
  ...
  \begin{solution}[3.5in]
    Let $Z$ denote the number of RVs among $X_1, X_2, X_3, X_4, X_5$ that take on a positive value.
    E.g.\ if all of them are positive then $Z$ is 4 and if none are positive then $Z$ is zero.
    Since $X_1, X_2, X_3, X_4, X_5$ are iid and $P(X_1 >0) = 3/4$, it follows that $Z \sim \mbox{ Binomial}(5,3/4)$ and we need to calculate $p(4)$ where $p(z) = {n\choose z} p^z(1-p)^{n-z}$.
    We have:
