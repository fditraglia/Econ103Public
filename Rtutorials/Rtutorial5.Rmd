...
### A Simple Example: Plotting $f(x) = x^2$
Let's start by plotting a few points on the curve $x^2$
```{r}
...
plot(x, y)
```
This is more like what we're after, but we'd rather have a smooth curve rather than all those little "dots." 
```{r}
plot(x, y, type = "l")
```
The option ``l`` stands for "line." 
***
### Exercise #1 - Plot $f(x) = x^2$ on $[-2,2]$.
...
```
***
### Exercise #2 - Plot $f(x) = log(x)$ on $[0.5, 1.5]$.
```{r cache=TRUE,echo=FALSE,results='hide',fig.show='hide',warning=FALSE}
#Exercise #2
...
```
***
### Exercise #3 - Plot $f(x) = sin(x)$ on $[0, 6\pi]$.
```{r cache=TRUE,echo=FALSE,results='hide',fig.show='hide',warning=FALSE}
#Exercise #3
...
```
***
### Exercise #4 - Plot $sin(x)$, $cos(x)$ and $2sin(x + \pi/4)$ on $[0,2\pi]$.
```{r cache=TRUE,echo=FALSE,results='hide',fig.show='hide',warning=FALSE}
#Exercise #4
...
```
***
## The Binomal Random Variable in R
...
### ``rbinom`` - Simulate Draws from a Binomial RV
...
```
***
### ``dbinom`` - Probability Mass Function for Binomial RV
...
```
***
### Exercise #6 - Plot the pmf of a Binomial$(n = 20, p = 0.65)$ Random Variable.
```{r cache=TRUE,echo=FALSE,results='hide',fig.show='hide',warning=FALSE}
#Exercise #6
support <- 0:20
...
```
***
### ``pbinom`` - Cumulative Distribution Function for a Binomial RV
