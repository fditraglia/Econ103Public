...
It's not much harder to find summaries grouped by two or more variables. Suppose  we wanted to calculate average height by sex *and* credits. We simply surround the argument to `by` with `.()` like so:

```{r by_multiple}
survey[ , mean(height, na.rm = TRUE), by = .(sex, credits)]
```
