...
## Part 1: What is Monte Carlo Simulation?
...
## Part 2: The Building Blocks of Monte Carlo Simulation in R
...
```
***
### Simulating Dice Rolls with ``sample``
...
Now let's try writing a slightly more general version of the ``two.dice`` function so we can see how to use ``replicate`` with a function that *takes its own arguments*. The function ``dice.sum2`` takes one argument ``n.dice`` that specifies *how many* six-sided dice we will roll and sum
```{r}
...
### An Important Point about Random Experiments in R
...
## Part 3: Approximating Probabilities with Monte Carlo Simulation
...
### Part 5: Using Logical Conditions to Calculate Probabilities from Simulation Results
...
```
***
### Exercise #8 - Simulating Pascal and Fermat
