...
In each case, plot the density or mass function of the population and compare it to the histograms of the sample mean computed for random samples drawn from that population based on 10,000 simulation replications.
...
You can use \texttt{curve} to plot both the uniform and Chi-squared densities, but you'll need another approach to plot the Bernoulli pmf.
I suggest that you create a vector \texttt{x} to represent the support set, and \texttt{p} to represent the pmf.
