...
%\question Let $X$ and $Y$ be discrete random variables and $a,b,c$ be constants. Prove that $E[aX + bY + c] = aE[X] + bE[Y]+c$. Hint: the steps are similar to slide 27 of Lecture \#8, but you'll have a \emph{double sum} over $x$ and $y$.  
%\begin{solution}
%
%\begin{eqnarray*}
		...
		\begin{solution}
			The First Order Condition is:
				...
				So we have
					$$\omega^* = \frac{\sigma_2^2 - \rho\sigma_1\sigma_2}{\sigma_1^2 + \sigma_2^2 -2\rho\sigma_1\sigma_2}$$
		\end{solution}
    \part \emph{Optional Part}: check the second order condition from part (e).
			\begin{solution}
...
\question We say that $X \sim \text{Exponential}(\lambda)$ if $X$ has probability density function $f(x) = \lambda e^{-\lambda x}$ for $x \in [0, +\infty)$, zero otherwise, and $\lambda > 0$.
  ...
  \begin{parts}
    \part Calculate the CDF.
    \part Calculate prob of a certain integral, both analytically and using numerical integration in R.
    \part Plot the PDF for some particular values of $\lambda$.
    \part Simulate and make histograms.
    \part Using the simulations, try to guess how the mean is related to $\lambda$.
    \part Use numerical integration to calculate the mean for particular values of Lambda.
    \part Hard part: calculate the mean of an arbitrary Exponential RV. Hint, you will need integration by parts.
  \end{parts}

\fullwidth{\section*{Lecture \#12 -- Discrete RVs IV}}

\question Some practice with quantile functions: calculate some, plot them, understand the relationship to a CDF.

\question A more interesting problem involving the normal distribution\dots
