...
\begin{parts}
  \part Without using R, what is the p-value for Linday's test? If $\alpha = 0.01$ would she reject the null hypothesis?
  \begin{solution}
...
\question Suppose that you wish to test $H_0\colon \mu = -1$ with a significance level of $\alpha = 0.1$. Your sample mean equals $-2.1$ with a standard error of $0.6$. 
To help you answer this question, note that $\texttt{qnorm(0.90)} \approx 1.28$ and $\texttt{qnorm(0.95)} \approx 1.64$.
\begin{parts}
...
\question Suppose that $X_1, \dots, X_9 \sim \text{iid N}(1, 4)$ and $Y_1, \dots, Y_{16} \sim \text{iid N}(-1, 9)$. Write a line of R code to calculate $P(\bar{X} - \bar{Y} > 0)$.

\question Let $X \sim N(2, 4)$, $Y \sim N(3,9)$, and define $Z = Y - X$. 
Now, suppose that $Z$ is normally distributed and $Corr(X,Y) = 0.5$. Write a line of R code to calculate $P(|Z|>1)$.

\question A matched-pairs hypothesis test from an old exam.

\question An independent samples hypothesis test from an old exam.
