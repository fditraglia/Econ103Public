...
\begin{parts}
  ...
  \begin{solution}
    Since $Z \sim N(\mu, \sigma^2/n) = N(-2, 1/4)$, 
    \[X = 2Z + 4 = 2(Z + 2) = \frac{Z-(-2)}{\sqrt{1/4}} \sim N(0,1) \]
  \end{solution}
  \part What is $P(|X|>1)$?
  \begin{solution}
    $P(|X|>1) = P(-1 \leq X \leq 1) = \texttt{pnorm(1) - pnorm(-1)} \approx 0.68$
  \end{solution}
\end{parts}
...
\uplevel{For the following two questions (Alice and Bob), you may assume any tests and confidence intervals are based on a random sample from a normal population with known variance.}
...
\todo[inline]{Need to be careful about the definition of the test statistic! Don't want to lose the sign information!}

\question Suppose that you wish to test $H_0\colon \mu = 4$ with a significance level $\alpha = 0.05$. Based on your sample data, you calculate a test statistic of $2.1$. 
\begin{parts}
  \part If the alternative hypothesis is $H_1\colon < 4$, would you reject $H_0$?
  \begin{solution}
    No: for this alternative we should only reject $H_0$ when the test statistic is a sufficiently large \emph{negative} number.
  \end{solution}
  \part If the alternative hypothesis is $H_1\colon > 4$, would you reject $H_0$?
  \begin{solution}
    Yes: the critical value for this test is less than 2, whereas our test statistic is greater than 2.
  \end{solution}
\end{parts}

\question Suppose that you wish to test $H_0\colon \mu = -1$ with a significance level of $\alpha = 0.1$. Based on your sample data, you calculate a test statistic of $-3.5$. 
\begin{parts}
...
\uplevel{The following 4 questions rely on a dataset called \texttt{mother\_child\_weight.csv} available from \texttt{http://ditraglia.com/econ103/}. The dataset contains two columns: \texttt{mother} gives a mother's weight during pregnancy (kg) while \texttt{child} give's her child's birthweight (grams).}
...
\uplevel{The next three questions rely on the regression results from the preceding question:}
\begin{verbatim}
           coef.est coef.se
(Intercept) 1.50     0.63   
mother      0.03     0.01   
