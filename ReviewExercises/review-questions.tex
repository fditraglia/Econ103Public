...
\todo[inline]{Some questions about R, given that there's a big R example in the slides. Also some questions about statistical significance, proportions, data-dredging, etc.}
...
\uplevel{The next ??? questions rely on the regression results from the preceding question:}
\begin{verbatim}
           coef.est coef.se
(Intercept) 1.50     0.63   
mother      0.03     0.01   
...
\question Interpret the estimated intercept from the preceding set of regression results.
\begin{solution}
...
\begin{parts}
  ...
  \part Suppose you wanted to test $H_0\colon \beta_1$ against the two-sided alternative at the 1\% significance level. Would you reject the null hypothesis?
\end{parts}
\todo[inline]{Something with tests and CIs for the regression results.}
...
\todo[inline]{This should test them on the varying slopes varying intercepts example from class, including the R commands, interpretation, inference, etc.}
