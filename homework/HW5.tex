...
\question Suppose $X$ is a random variable with support $\{-1, 0, 1\}$ where $p(-1)=q$ and $p(1) = p$.
	\begin{parts}
		...
		\part Calculate the CDF, $F(x_0)$, of X.
			...
			\end{solution}
		\part Calculate $\expect[X]$.
		\begin{solution}$\expect[X] = -1 \cdot q + 0 \cdot (1-p-q) + p\cdot 1 = p-q$ \end{solution}
		\part What relationship must hold between $p$ and $q$ to ensure $\expect[X]=0$?
...
\end{center}
		...
		\item Calculate the conditional probability distribution of $Y|X=1$ and $Y|X=2$.
			...
			The distribution of $Y|X = 1$ is
				...
				\end{eqnarray*}
			\end{solution}
		\item Calculate $\expect[Y|X]$.
			\begin{solution}
			\begin{eqnarray*}
				\expect[Y | X =1 ] &=& 0 \times 0.4 + 1 \times 0.6 = 0.6\\
				\expect[Y | X =2 ] &=& 0 \times 0.5 + 1 \times 0.5 = 0.5
			\end{eqnarray*}
			Hence, 
				$$\expect[Y|X] = \left\{ \begin{array}{ll} 0.6  & \mbox{with probability } 0.5\\ 0.5& \mbox{with probability } 0.5\end{array} \right.$$
				since $p_X(1) = 0.5$ and $p_X(2) = 0.5$.
			\end{solution}
		\item What is $\expect[\expect[Y|X]]$?
			\begin{solution}
			$\expect[\expect[Y|X]] = 0.5 \times 0.6 + 0.5 \times 0.5 = 0.3 + 0.25 = 0.55$. Note that this equals the expectation of $Y$ calculated from its marginal distribution, since $\expect[Y] = 0 \times 0.45 + 1 \times 0.55$. This illustrates the so-called ``Law of Iterated Expectations.''
			\end{solution}
		\item Calculate the covariance between $X$ and $Y$ using the shortcut formula.
		\begin{solution}
		First, from the marginal distributions, $E[X] = 1\cdot 0.5 + 2 \cdot 0.5 = 1.5$ and $\expect[Y]=0 \cdot 0.45 + 1 \cdot 0.55 = 0.55$. Hence $\expect[X]\expect[Y] = 1.5 \cdot 0.55 = 0.825$. Second,
			\begin{eqnarray*}
				\expect[XY] &=& (0\cdot 1) \cdot 0.2 + (0\cdot 2)\cdot 0.25 + (1\cdot 1) \cdot 0.3 + (1\cdot 2) 0.25\\
						&=& 0.3 + 0.5 = 0.8
			\end{eqnarray*}
			Finally $Cov(X,Y) = \expect[XY] - \expect[X]\expect[Y] = 0.8 - 0.825 = -0.025$
		\end{solution}
	\end{parts}
