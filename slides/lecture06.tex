...
\hlstd{draw_simulation} \hlkwb{<-} \hlkwa{function}\hlstd{() \{}
  \hlstd{cards} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{'GG'}\hlstd{,} \hlstr{'OG'}\hlstd{,} \hlstr{'OO'}\hlstd{)}
  \hlstd{random_card} \hlkwb{<-} \hlkwd{sample}\hlstd{(cards,} \hlkwc{size} \hlstd{=} \hlnum{1}\hlstd{)}
  \hlkwa{if}\hlstd{(random_card} \hlopt{==} \hlstr{'GG'}\hlstd{) \{}
    \hlstd{faces} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{'G'}\hlstd{,} \hlstr{'G'}\hlstd{)}
  \hlstd{\}} \hlkwa{else if} \hlstd{(random_card} \hlopt{==} \hlstr{'OO'}\hlstd{) \{}
    \hlstd{faces} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{'O'}\hlstd{,} \hlstr{'O'}\hlstd{)}
  \hlstd{\}} \hlkwa{else} \hlstd{\{}
    \hlstd{faces} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{'O'}\hlstd{,} \hlstr{'G'}\hlstd{)}
  \hlstd{\}}
  \hlstd{out} \hlkwb{<-} \hlkwd{sample}\hlstd{(faces)}
  \hlkwd{names}\hlstd{(out)} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{'front'}\hlstd{,} \hlstr{'back'}\hlstd{)}
...
\end{verbatim}
\begin{alltt}
\hlstd{Obama_on_front} \hlkwb{<-} \hlkwd{subset}\hlstd{(simulations, front} \hlopt{==} \hlstr{'O'}\hlstd{)}
\hlkwd{mean}\hlstd{(Obama_on_front}\hlopt{$}\hlstd{back} \hlopt{==} \hlstr{'O'}\hlstd{)}
\end{alltt}
\begin{verbatim}
## [1] 0.6633065
\end{verbatim}
